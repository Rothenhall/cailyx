...
/**
 * ErrorBoundary — /v3. Wraps the canvas so one card throwing on a malformed
 ...
 * class component.
 *
 * @module app/v3/_components/ErrorBoundary
 */
...
export class ErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    ...
    console.error(`[v3] ${this.props.label} failed to render`, error, info.componentStack);
  }
      ...
      <div className="grid h-full w-full place-items-center p-4">
        <div className="max-w-[280px] rounded-r3 border border-border bg-bg-raised/90 p-4 text-center shadow-e2">
          ...
          <button
            ...
            className="mt-3 rounded-r2 border border-accent-dim bg-accent-dim/14 px-3 py-1.5 text-caption font-medium text-accent transition-colors duration-micro hover:bg-accent-dim/24"
          >
            Try again
          </button>
        </div>

...
/**
 * SettingsPanel — /v3. One grouped overlay for the header actions: Connections
 ...
 *
 * @module app/v3/_components/SettingsPanel
 */
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-start justify-center p-4 pt-[7vh]">
      ...
      <div
        ...
        className="v3-pop relative flex max-h-[84vh] w-full max-w-xl flex-col overflow-hidden rounded-r4 border border-border bg-bg-raised shadow-e3"
      >
        {/* header */}
              ...
              {ops === null ? (
                <ul className="space-y-1.5">
                  {Array.from({ length: 3 }, (_, i) => (
                    <li key={i} className="v3skel h-[74px] rounded-r3" />
                  ))}
                </ul>
              ) : (
                <ul className="space-y-1.5">

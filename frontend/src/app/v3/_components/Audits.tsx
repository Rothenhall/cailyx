...
/**
 * Audits — /v3. A chrome-less card between the Flywheel and the Agents Feed.
 *
 ...
 *
 * @module app/v3/_components/Audits
 */
...
import { band, tone as toneOf, type ToneKind } from '@/app/v3/_lib/audit';
        ...
        {skeleton ? (
          <div className="space-y-1.5 p-3">
            {Array.from({ length: 5 }, (_, i) => (
              <div key={i} className="v3skel h-7 rounded-r2" />
            ))}
          </div>
        ) : !projectId ? (
          <div className="p-3">
          ...
          <div className="v3-stagger flex flex-col gap-2 p-3">
            {TABS.map((t, i) => {
                ...
                >
                  <span className="min-w-0 flex-1">
                    <span className="flex items-center gap-1.5">
                      ...
                      {m.warn && <span className="v3-dot v3-dot-attention shrink-0" />}
                    </span>
          ...
          <div key={tab} className="v3tab-in p-3">
            {tab === 'seo' && (

...
import { promptLimitForTier, tierSlugFromOfferText } from './plan-tier.util';
import type {
...
export class PromptRequestsService {
  ...
  /**
   * §20 — resolve the client's plan tier from their most recent live
   * subscription's `Offer.code`/`Offer.name`. See `plan-tier.util.ts` for
   * why this is a derived judgment call, not a stored field.
   */

...
Scale 1,000 / Enterprise unlimited). **Resolved on merge:** this module was
first built when no model had a clean, queryable "plan tier" field, so it
derived one from the client's most recent live `Subscription` → its
`Offer.code`/`Offer.name` (the field observed transiently on the shared dev
database during this build's own testing was Phase C7's concurrent work,
merged separately). `Client.planTier` is now real and merged —
`resolvePlanTier()` reads it directly, falling back to the original
Offer-derivation only if the stored value is somehow unrecognized
(`plan-tier.util.ts`'s `isPlanTierSlug` guard). No matching tier still
defaults to **Starter** (the most conservative limit).

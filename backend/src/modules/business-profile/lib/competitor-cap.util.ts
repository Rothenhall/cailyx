 ...
 * dedicated, distinguishable error the frontend renders as an upsell moment
 * rather than a generic validation failure. See
 * `BusinessProfileService.saveDraft`'s cap check and
...
/** Normalize an arbitrary `Client.planTier` string to a known tier slug, defaulting to the most conservative (`starter`) for anything unrecognized — never silently grants a higher limit than the client is known to be on. */
